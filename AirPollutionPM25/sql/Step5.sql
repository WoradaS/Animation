...
SELECT * FROM AirPollutionPM25
  WHERE longitude IN ( SELECT MAX(longitude)
                       FROM dbo.AirPollutionPM25 
                       WHERE years = 2016 AND county = 'Thailand'
                       GROUP BY county, years ) or
		longitude IN ( SELECT MIN(longitude) 
                       FROM dbo.AirPollutionPM25 
                       WHERE years = 2016 AND county = 'Thailand'
                       GROUP BY county, years ) or
        latitude  IN ( SELECT MIN(latitude)  
                       FROM dbo.AirPollutionPM25 
                       WHERE years = 2016 AND county = 'Thailand'
                       GROUP BY county, years ) or
        latitude  IN ( SELECT MAX(latitude)  
                       FROM dbo.AirPollutionPM25 
                       WHERE years = 2016 AND county = 'Thailand'
...
select county,city,pm25,latitude,longitude,years FROM dbo.AirPollutionPM25 
WHERE county in(select county FROM dbo.AirPollutionPM25 
                WHERE years = 2011 AND pm25 in ( select Max(pm25) 
                                                 FROM dbo.AirPollutionPM25 
                                                 where years = 2011 
                                               )
                ) 
