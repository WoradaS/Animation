...
def index():
   cursor = conn.cursor()
   cursor.execute('SELECT * FROM master.dbo.AirPollutionPM25')
...
def delete(county,city,years,latitude,longitude):
    if request.method == "GET" :        
      with conn.cursor() as cursor:
        cursor.execute("DELETE FROM master.dbo.AirPollutionPM25 where county=? AND city=? AND years=? AND latitude=? AND longitude=? ",(county,city,years,latitude,longitude))
        conn.commit()
...
def add():
   if request.method == "POST" :
       ...
       if(pm25<=10):
         ...
         conc_pm25 = '>50'
       with conn.cursor() as cursor:
              sql = "Insert into master.dbo.AirPollutionPM25(county,city,years,pm25,latitude,longitude,populations,wbinc16_text,region,conc_pm25,color_pm25) values(?,?,?,?,?,?,?,?,?,?,?)"
...
def insertCSV():  
   ...
   with conn.cursor() as cursor:
      for row in df.itertuples():
          ...
          cursor.execute( "INSERT INTO master.dbo.AirPollutionPM25(county,city,years,pm25,latitude,longitude,populations,wbinc16_text,Region,conc_pm25,color_pm25) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(row.country,row.city,years,pm25,latitude,longitude,populations,row.wbinc16_text,row.Region,row.conc_pm25,row.color_pm25))
          conn.commit()
      conn.commit()
      return redirect(url_for('index'))
...
def citypoints():
   if request.method == "POST" : 
        ...
        with conn.cursor() as cursor:
           cursor.execute('SELECT county,city,pm25,latitude,longitude FROM master.dbo.AirPollutionPM25 WHERE years = ? for JSON PATH ' ,(years))
           mydata = cursor.fetchall()
           cursor.execute('SELECT count(*) FROM master.dbo.AirPollutionPM25 WHERE years = ?  ' ,(years))
...
def income():
   if request.method == "POST" : 
        ...
        with conn.cursor() as cursor:
           cursor.execute("SELECT county,city,pm25,latitude,longitude FROM master.dbo.AirPollutionPM25 WHERE years = ? AND wbinc16_text = 'Low income' for JSON PATH " ,(years))
           mydata = cursor.fetchall()
           cursor.execute("SELECT count(*) FROM master.dbo.AirPollutionPM25 WHERE years = ? AND wbinc16_text = 'Low income' " ,(years))
           countdata = cursor.fetchall()
           jsonn = ""
...
@app.route('/MBR')
def MBR(): 
        tiltlehighestno = 'MBR covering all city points in Thailand in 2016'
        cursor = conn.cursor()
        sql = "SELECT  county,city,pm25,latitude,longitude FROM AirPollutionPM25 WHERE longitude IN ( SELECT MAX(longitude) FROM dbo.AirPollutionPM25 WHERE years = 2016 AND county = 'Thailand' GROUP BY county, years ) or longitude IN ( SELECT MIN(longitude) FROM dbo.AirPollutionPM25 WHERE years = 2016 AND county = 'Thailand'GROUP BY county, years ) or latitude  IN ( SELECT MIN(latitude)  FROM dbo.AirPollutionPM25 WHERE years = 2016 AND county = 'Thailand'GROUP BY county, years ) or latitude  IN ( SELECT MAX(latitude)  FROM dbo.AirPollutionPM25 WHERE years = 2016 AND county = 'Thailand' GROUP BY county, years )  for JSON PATH "
        cursor.execute(sql)
        mydata = cursor.fetchall()
        jsonn = ""
        for row in mydata:
